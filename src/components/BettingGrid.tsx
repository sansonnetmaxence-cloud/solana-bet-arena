...
const BettingGrid = ({ currentPrice, previousPrice, priceHistory, onSelectBet, activeBets, betResult, quickBetMode, countdown, selectedPrice, selectedDirection, market }: BettingGridProps) => {
  ...
  return (
    <div className={cn(
      ...
      {/* Desktop layout */}
      <div className="hidden sm:block max-h-[80vh] overflow-y-auto overflow-x-hidden scrollbar-hide relative z-10 rounded-2xl bg-card/30 border border-border/20 p-3 md:p-4">
      ...
      <div className="hidden sm:block max-h-[80vh] overflow-y-auto overflow-x-visible scrollbar-hide relative z-10 rounded-2xl bg-card/30 border border-border/20 p-2 md:p-3">
        <div className="grid grid-cols-4 md:grid-cols-5 gap-1 md:gap-1.5">
          {upPrices.map((p, i) => renderCard(p, 'up', i))}
        </div>
      </div>
